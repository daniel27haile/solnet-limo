...
import { Testimonial } from '../../../shared/components/testimonial-card/testimonial-card.component';
...
@Component({
  ...
  imports: [
    ...
    TestimonialsCarouselComponent, LoadingSpinnerComponent,
  ],
...
export class HomeComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.fleetService.getFleet().subscribe({
      next: (data) => {
        this.fleet.set((data.length ? data : FALLBACK_FLEET).slice(0, 3));
        this.loadingFleet.set(false);
      },
      error: () => {
        this.fleet.set(FALLBACK_FLEET.slice(0, 3));
        this.loadingFleet.set(false);
      },
    });
  }
